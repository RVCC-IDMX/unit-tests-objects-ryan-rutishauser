...
function getThreeBandValue(bands) {
  const colorValue = ((getColorValue(bands.color1) * 10) + getColorValue(bands.color2));
  return (colorValue * getMultiplierValue(bands.multiplier));
}
...
function getResistorOhms(bands) {
  const numString = formatNumber(getThreeBandValue(bands));
  const resPercent = getTolerance(bands.tolerance);
  return `Resistor value: ${numString} Ohms ${resPercent}`;
}
